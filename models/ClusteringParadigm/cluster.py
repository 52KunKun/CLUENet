...
class Clustering(nn.Module):
    ...
    def forward(self, x): #[b,c,w,h]
        ...
        # feature[B,T,C]  [B,1,T,C]  [B,C_T,T,1]--[B,C_T,T,C]
...
class Cluster(nn.Module):
    def __init__(self, layers, embed_dims = None,
        ...
        for i in range(len(layers)):
            ...
            if i >= len(layers) - 1: # 最后一个阶段不加入下采样
                break
...
# 改变框架降低参数为指定大小
class cluster_mini_cifar(Cluster):
    def __init__(self, **kwargs):
        layers = [2, 2, 2, 2]
        norm_layer = GroupNorm
        ...
        down_pad =1
        super().__init__(
            layers,in_patch_size=in_patch_size,in_stride=in_stride,in_pad=in_pad, embed_dims=embed_dims, norm_layer=norm_layer,
            mlp_ratios=mlp_ratios, downsamples=downsamples,
            down_patch_size = down_patch_size, down_pad=down_pad,
            center_w=center_w, center_h=center_h, window_w=window_w, window_h=window_h,
            heads=heads, head_dim=head_dim,
            fork_feat=False, return_center = False, num_clustering = 3,drop_path_rate=0.1,
            **kwargs)



# 改变框架降低参数为指定大小
class cluster_mini(Cluster):
    def __init__(self, **kwargs):
        layers = [3, 4, 5,2]
        norm_layer=GroupNorm
        ...
        super().__init__(
            ...
            center_w=center_w, center_h=center_h, window_w=window_w, window_h=window_h,
            heads=heads, head_dim=head_dim,
            fork_feat=False, return_center = False, num_clustering = 3,drop_path_rate=drop_path_rate,
            **kwargs)

class cluster_mini_vis(Cluster):
    def __init__(self, **kwargs):
        layers = [3, 4, 5,2]
        norm_layer=GroupNorm
        embed_dims = [32, 64, 196, 320]
...
class cluster_tiny(Cluster):
    def __init__(self, **kwargs):
        ...
        super().__init__(
            ...
            fork_feat=False, return_center = False, num_clustering = 3,
            **kwargs)
...
def get_flops_ptflops_method(model,input_size):
    ...
    # tiny : flops:  5.6 GMac params:  30.97 M
    # small: flops:  9.78 GMac params:  52.27 M
if __name__=='__main__':
    model = cluster_tiny(num_classes=100)
    get_flops_ptflops_method(model,input_size=(3,224,224))
    # tiny:29.53M
    # small:49.85M