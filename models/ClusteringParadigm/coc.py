...
#         # sim = sim * mask
#         sim = (centers.reshape(b,c,-1).transpose(-1,-2)@x.reshape(b,c,-1))*self.scale
...
#         out = (sim@value2+value_centers)/(sim.sum(dim=-1, keepdim=True) + 1.0)
#
...
def coc_mini_cifar(pretrained=False, **kwargs):
    layers = [2, 2, 2, 2]
    ...
    model = ContextCluster(
        ...
        proposal_w=proposal_w, proposal_h=proposal_h, fold_w=fold_w, fold_h=fold_h,
        heads=heads, head_dim=head_dim,in_patch_size=3,in_stride=1,in_pad=1,drop_path_rate=0.1,
        **kwargs)
...
def coc_tiny(pretrained=False, **kwargs):
    ...
    model = ContextCluster(
        ...
        down_patch_size=down_patch_size, down_pad=down_pad,
        proposal_w=proposal_w, proposal_h=proposal_h, fold_w=fold_w, fold_h=fold_h,
        heads=heads, head_dim=head_dim,drop_path_rate=drop_path_rate,
        **kwargs)
    model.default_cfg = default_cfgs['model_small']
    return model

# def coc_tiny_cifar(pretrained=False, **kwargs):
#     layers = [3, 4, 5, 2]
#     norm_layer = GroupNorm
#     embed_dims = [32, 64, 196, 320]
#     mlp_ratios = [8, 8, 4, 4]
...
#     down_pad = 1
#     model = ContextCluster(
#         layers, embed_dims=embed_dims, norm_layer=norm_layer,in_patch_size=in_patch_size,in_stride=in_stride,in_pad=1,
#         mlp_ratios=mlp_ratios, downsamples=downsamples,
#         down_patch_size=down_patch_size, down_pad=down_pad,
#         proposal_w=proposal_w, proposal_h=proposal_h, fold_w=fold_w, fold_h=fold_h,
...
def coc_medium(pretrained=False, **kwargs):
    ...
    model = ContextCluster(
        layers, embed_dims=embed_dims, norm_layer=norm_layer,
        mlp_ratios=mlp_ratios, downsamples=downsamples,
        down_patch_size=down_patch_size, down_pad=down_pad,drop_path_rate=drop_path_rate,
        proposal_w=proposal_w, proposal_h=proposal_h, fold_w=fold_w, fold_h=fold_h,
        heads=heads, head_dim=head_dim,
        **kwargs)
    model.default_cfg = default_cfgs['model_small']
...
def get_flops_method2(model,input_size):
    from thop import profile
    # 使用 thop 库计算 FLOPs 和参数量
    B = 1
    input_tensor = torch.randn(B,input_size[0],input_size[1],input_size[2])  # 假设 input_size 是你的输入张量形状
    flops, params = profile(model, inputs=(input_tensor,))
    print(f"模型的参数量：{params / 1e6:.2f}M")
    print(f"模型的FLOPs：{flops / 1e9/B:.2f}G")


if __name__ == '__main__':
    model = coc_medium(num_classes=100)
