...
class Cluster(nn.Module):
    ...
    def forward(self, x):  # [b,c,w,h]
        ...
        # dispatch step, return to each point in a cluster  [B,M,1,D]*[B,1,N,D]  [B,M,N,D]
    ...
    def __init__(self, layers, embed_dims=None,
        ...
        for i in range(len(layers)):
            ...
            if downsamples[i] or embed_dims[i] != embed_dims[i + 1]:
                ...
                network.append(ClusterPool(down_stride, embed_dims[i], embed_dims[i + 1], fold_w=fold_w[i + 1],
                                           fold_h=fold_h[i + 1]))
...
def fec_mini_cifar(pretrained=False, **kwargs):
    layers = [2, 2, 2, 2]
...
def fec_small(pretrained=False, **kwargs):
    ...
    model = FEC(
        ...
        down_patch_size=down_patch_size, down_pad=down_pad,
        proposal_w=proposal_w, proposal_h=proposal_h, fold_w=fold_w, fold_h=fold_h,
        heads=heads, head_dim=head_dim,drop_path_rate =drop_path_rate, **kwargs)
    model.default_cfg = default_cfgs['model_small']
    return model
...
def fec_large(pretrained=False, **kwargs):
    ...
    model = FEC(
        ...
        mlp_ratios=mlp_ratios, downsamples=downsamples,
        down_patch_size=down_patch_size, down_pad=down_pad,
        proposal_w=proposal_w, proposal_h=proposal_h, fold_w=fold_w, fold_h=fold_h,drop_path_rate=drop_path_rate,
...
if __name__ == '__main__':
    ...
    # model = fec_small(num_classes=100)
    # out = model(input)
    # print(model)
    # print(out.shape)
    # n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    # print("number of params: {:.2f}M".format(n_parameters / 1024 ** 2))
    # from models.ablationStudy.ClusterPVT import get_flops_ptflops_method
    ...
    model = fec_large(num_classes=1000)
    get_flops_ptflops_method(model, (3, 224, 224))
