...
class PatchEmbedFirst(nn.Module):
    # 初始使用3的卷积层
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(len(depths)):
            if i == 0:
                self.patch_embeds.append(PatchEmbedFirst(img_size, patch_size, in_chans, embed_dims[i]))
            else:
...
def pcpvt_b0(pretrained=False, **kwargs):
    model = CPVTV2(
...
if __name__ == '__main__':
    model = pcpvt_b0(num_classes=100)
