...
def get_attention_score(self, input, output):
    ...
    if self.window_w > 1 and self.window_h > 1:
        b0, c0, w0, h0 = x.shape
        assert w0 % self.window_w == 0 and h0 % self.window_h == 0, \
            f"Ensure the feature map size ({w0}*{h0}) can be divided by fold {self.window_w}*{self.window_h}"
        x = rearrange(x, "b c (f1 w) (f2 h) -> (b f1 f2) c w h", f1=self.window_w,
                      f2=self.window_h)  # [bs*blocks,c,ks[0],ks[1]]
        value = rearrange(value, "b c (f1 w) (f2 h) -> (b f1 f2) c w h", f1=self.window_w, f2=self.window_h)
        feature = rearrange(feature, "b c (f1 w) (f2 h) -> (b f1 f2) c w h", f1=self.window_w, f2=self.window_h)
    ...
    feature = rearrange(feature,"b c w h -> b (w h) c")

    # processing before flash attention
    centers = rearrange(centers,"(b hd) n c -> b n hd c",hd=self.heads).type(torch.half)
    value = rearrange(value, "(b hd) n c -> b n hd c", hd=self.heads).type(torch.half)
    feature = rearrange(feature, "(b hd) n c -> b n hd c", hd=self.heads).type(torch.half)
    ...
    centers = rearrange(centers,"b (w h) hd c -> b (hd c) w h",w=c_w,h=c_h).type(torch.float)
...
if __name__ == '__main__':
    ...
    args.model = "cluster_mini"
    args.shape = 224
    args.num_classes = 100
    args.checkpoint = "./ClusterFormer_mini_epoch105.pth"
    args.alpha = 1.0
    args.num_clu = 5
