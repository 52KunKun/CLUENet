...
def infer(layer_idx,head):
    ...
    os.makedirs(f"images_CIFAR100/vit_cifar2/{image_name}", exist_ok=True)
    ...
    attn_map = attention_maps[0][:,head,:]  # shape: [H, h, w]，取某一 head 的注意力图
    attn_map = attn_map.unsqueeze(0)  # [1,1,H,W]
    attn_map = F.interpolate(attn_map, size=(h, w), mode='bilinear', align_corners=False)  # 插值为原图大小
    ...
    # 转为热力图（伪彩色）
    attn_heatmap = cv2.applyColorMap(attn_map, cv2.COLORMAP_JET)  # shape: (H,W,3)，uint8

    # 保存图像
    save_path = f"images_CIFAR100/vit_cifar2/{image_name}/block{layer_idx}_head{head}_heatmap.png"
    cv2.imwrite(save_path, attn_heatmap)
