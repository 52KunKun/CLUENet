...
parser.add_argument('--data_path', type=str, default=r'D:\datasets\dataset\CIFAR100', help='path to imageFolder')
parser.add_argument('--image', type=int, default=2, help='path to image under the data_path')

parser.add_argument('--model', default='CLUA_mini_CIFAR', type=str, metavar='MODEL', help='Name of model')
...
parser.add_argument('--checkpoint', type=str, default="./CLUA_mini_CIFAR100_epoch105.pth", metavar='PATH',
...
dataset = torchvision.datasets.CIFAR100(root=args.data_path, train=False, download=True)
...
def main():
    ...
    model = CLUA.CLUA_mini_CIFAR(num_classes=args.num_classes)
    ...
    image_class_idx = dataset[args.image][1]
    print(f"this image true class is {object_categories[image_class_idx]}")
    ...
    stage_mask_length = len(mask)  # 记录开始索引
    # 检查model_num_clu
    # 首先确保 args.num_clu 不为 None
    # 这里的 args.model_num_clu 现在肯定是一个 list，不用再判断了
    ...
    # 调整颜色亮度（转为RGB+调整颜色亮度）
    image_name = f"dataset[{args.image}]"
    if isModelTrue:
    ...
    for stage in range(4):
        for head in range(heads[stage]):
            ...
            save_path = f"images_CIFAR100/{args.model}/{image_name}/stage{stage}_head{head}_{mask.shape[0]}CLU.png"
            # 只能保存BGR图像
    ...
    for num_clu in args.model_num_clu:
        ...
        # 转 BGR 保存 OpenCV
        img_with_masks = cv2.cvtColor(img_with_masks, cv2.COLOR_RGB2BGR)
        save_path = f"images_CIFAR100/{args.model}/{image_name}/all_stages_{num_clu}CLU.png"
        cv2.imwrite(save_path, img_with_masks)
        print(f"==> Generated image is saved to: {save_path}")
    # 原始图像仅保存一次
...
if __name__ == '__main__':
    ...
    image_index_list = [
        9,16,19,36,78,130,212
    ] # test

    args.model = "CLUA_mini_CIFAR"
    args.scale = 8
    args.num_classes = 100
    args.checkpoint = "./CLUA_mini_CIFAR100_epoch105.pth"
    args.alpha = 1.0
    args.num_clu = 16
    # args.model_num_clu = 3
    args.model_num_clu = [3,5,7,9,12,16]
