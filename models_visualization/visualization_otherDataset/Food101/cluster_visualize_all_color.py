...
# 1. 初始化
# idx -> name (列表，假设是1000类)
object_categories = ["" for _ in range(1000)]
# name -> idx (字典)
name_to_idx = {}

# 2. 读取 TXT 文件
with open(r"./data/classes2label.txt", "r", encoding="utf-8") as f:
    # 使用计数器 idx，从 0 开始
    idx = 0
    ...
    for line in f:
        ...
        # 跳过空行（防止文件末尾有多余空行导致计数错误）
        if not class_name:
            continue

        # 确保不越界（如果文件行数超过1000，后面的忽略；或者你可以把 object_categories 改为动态 append）
        if idx < len(object_categories):
            # 建立 idx -> name 映射
            object_categories[idx] = class_name
...
parser.add_argument('--data_path', type=str, default=r'D:\datasets\dataset\food-101.tar\food-101\images', help='path to imageFolder')
...
if __name__ == '__main__':
    ...
    def read_txt_file(txt_path, name_to_idx):
        """
        Args:
            txt_path: txt 文件路径，每行一个图片路径，如 "waffles/888311.jpg"
            name_to_idx: 字典，映射 {类别名: 索引}，例如 {'waffles': 382, ...}
        """
        images = []
        labels = []
        with open(txt_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                # ============================================================
                # 核心逻辑修改：从路径中提取类名 -> 查表得到 Label
                # ============================================================

                # 1. 统一将反斜杠替换为正斜杠 (兼容 Windows 路径)
                # "waffles\888311" -> "waffles/888311"
                temp_path = img_path.replace('\\', '/')

                # 2. 提取第一个分隔符前的字符串
                # "waffles/888311" -> parts=['waffles', '888311'] -> class_name='waffles'
                ...
                if len(parts) > 0:
                    ...
                    if class_name in name_to_idx:
                        ...
                        # 如果 TXT 里有的类别在你的 label2class 表里找不到，可以选择跳过或报错
                        # print(f"Warning: 类别 '{class_name}' 未在映射表中找到，跳过图片: {img_path}")
                        pass

        return images, labels
    images,labels = read_txt_file("./data/train.txt",name_to_idx)
    # 1. 确保数据量足够
    sample_num = 10
    total_len = len(images)

    if total_len < sample_num:
        print(f"数据量不足 {sample_num} 条，全部取出")
        sample_indices = range(total_len)
    else:
        # 2. 生成 10 个不重复的随机索引
        sample_indices = random.sample(range(total_len), sample_num)

    # 3. 根据索引取出对应的图片和标签
    image_path_list = [images[i] for i in sample_indices]
    label_list = [labels[i] for i in sample_indices]

    args.model = "CLUA_tiny_compare"
    args.shape = 224
    args.num_classes = 101
    args.checkpoint = r"C:\Users\13779\Desktop\pth\Food101\CLUA\CLUA_tiny_mini_epoch105.pth"
    args.alpha = 1.0
    args.num_clu = 16
