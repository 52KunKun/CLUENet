...
@torch.no_grad()
def evaluate_test_amp(model, data_loader, device,amp=False):
    """
    model: 待测试模型
    ...
    for step, data in enumerate(data_loader):
        ...
        sum_num_top1 += torch.eq(top1_pred, labels).sum()
        # top-3
        top3_pred = pred.topk(3, dim=1, largest=True, sorted=True)[1]
        sum_num_top3 += torch.sum(torch.eq(top3_pred, labels.unsqueeze(1).expand_as(top3_pred)), dim=1).sum()
    end_time = time.time()
    total_time = end_time - start_time
    total_fps = num_samples / total_time
    print(f"Total time for evaluation: {total_time:.2f} seconds")
